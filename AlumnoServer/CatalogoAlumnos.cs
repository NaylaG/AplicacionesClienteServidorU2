...
namespace AlumnoServer
{
    public class CatalogoAlumnos
    {
        ...
        {
            var alumno = Alumnos.FirstOrDefault(X => X.NumeroControl == a.NumeroControl); //num contl campo llave
            if(alumno!=null)
            {
        ...
        private void Cargar()
        {
            ...
            {
                var nueva = JsonConvert.DeserializeObject<ObservableCollection<Alumno>>(File.ReadAllText("alumnos.json"));
